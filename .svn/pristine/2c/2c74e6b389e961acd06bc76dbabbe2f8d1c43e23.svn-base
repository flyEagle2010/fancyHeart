...
void BattleScene::createSkill(int skillID,int pos)
{
    ImageView* rim=(ImageView*)skillNode->getChildByName("skill"+Value(pos).asString());
    ImageView* bar=(ImageView*)skillNode->getChildByName("bar"+Value(pos).asString());
    this->skillNode->removeChild(rim);
    
    SkillIcon* icon=SkillIcon::create(rim,bar->getPosition(),skillID);
...
void BattleScene::interceptTouchEvent(Widget::TouchEventType event, Widget *sender, Touch *touch)
{
    ...
    if(icon && icon->isOK){
        // 使用技能
        BattleMgr::getInstance()->skillAttack(icon->skillID);
        icon->start();
